...
- Default app data should live in `~/Library/Application Support/Connectum/Local/store.json`; credentials and OAuth tokens should live in macOS Keychain.
- The intended repository path is local-first. If the current branch still has hosted `CrmRepository` code, treat it as legacy/in-progress and do not build new features on hosted infrastructure without an explicit product decision.
