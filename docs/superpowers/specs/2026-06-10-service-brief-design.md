...
### 5.3 `ai-chat` 주입 변경
`ai-chat/index.ts`의 `systemPrompt()`에 브리프를 **새 캐시 블록**으로 추가한다(기존 정량 overview 블록은 유지). 브리프가 비어있으면(`status='empty'`) 주입하지 않는다. `draft`/`confirmed` 모두 주입(없는 것보다 나음). `cache_control: ephemeral` 적용.
...
- `status`: `draft`=AI 생성 후 유저 미확인, `confirmed`=유저가 보고 수락. 엣지 함수는 `adminClient()`(service role)로 접근하므로 RLS는 기존 service-scoped 테이블과 동일 정책.
